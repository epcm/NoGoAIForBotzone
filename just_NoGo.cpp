...
bool State::isTerminal()
{
    for (int i = 0; i < 9; i++)
        for (int j = 0; j < 9; j++)
            if (current_board[i][j] == 0 && judgeAvailable(i, j, col))
                return false;
    return true;
}
...
{
    return children.size() == state.available_choices.size();
}

//使用UCB算法，权衡exploration和exploitation后选择得分最高的子节点，注意如果是预测阶段直接选择当前Q值得分最高的。
Node *bestChild(Node *node, bool is_explor)
{
    double max_score = 2e-300;
...
int main()
{
    ...
    int it = 0;
    Node *node = new Node;
    if (input["requests"][it]["x"].asInt() == -1) //确定颜色
        node->state.col = 1;
    else
        node->state.col = -1;
